@php
    ...
    $menus = [['route' => 'admin.dashboard', 'label' => 'Dashboard'], ['route' => 'admin.users.index', 'label' => 'Data Pengguna'], ['route' => 'admin.withdrawal.index', 'label' => 'Withdrawal Mentor'], ['route' => 'admin.slider', 'label' => 'Slider Banner'], ['route' => 'admin.faq', 'label' => 'FAQ'], ['route' => 'admin.settings.edit', 'label' => 'Setting Web']];
@endphp

<nav x-data="{ open: false }" class="border-b border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800">
    {{-- Bar Atas: Logo & User Control --}}
            ...
            <div class="flex items-center gap-2 sm:gap-4">
                @auth
                    ...
                    <div class="shrink-0 sm:hidden">
                        @if (Auth::user()->avatar_url)
                            <img src="{{ asset('storage/' . Auth::user()->avatar_url) }}" class="h-8 w-8 rounded-full border-2 border-[#20C896] object-cover">
                        @else
                ...
                {{-- Hamburger Button (Trigger Utama) --}}
    ...
    <div x-show="open" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 -translate-y-2" x-transition:leave="transition ease-in duration-150" class="border-t border-white/10 bg-[#20C896] sm:hidden" x-cloak>

        <div class="bg-black/5 px-4 py-4">
            <p class="text-[10px] font-black uppercase tracking-widest text-white/50">Admin Menu</p>
        </div>

        <div class="space-y-1 p-2">
            @foreach ($menus as $menu)
                <x-responsive-nav-link :href="route($menu['route'])" :active="request()->routeIs($menu['route'])" class="{{ request()->routeIs($menu['route']) ? 'bg-white/20' : '' }} block rounded-lg border-none py-3 text-[10px] font-black uppercase tracking-widest !text-white hover:bg-white/10">
                    {{ __($menu['label']) }}
            ...
            <x-responsive-nav-link :href="route('admin.profile.edit')" class="block border-none py-3 text-[10px] font-black uppercase tracking-widest !text-white opacity-80">
                {{ __('Profil Admin') }}
            </x-responsive-nav-link>

            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit" class="w-full rounded-lg px-4 py-3 text-left text-[10px] font-black uppercase tracking-widest text-red-200 hover:bg-red-500/10">
                    {{ __('Keluar Panel') }}
                </button>
            </form>
        </div>
