<x-app-layout>
    <div class="mx-auto max-w-[1280px] px-4 py-10" role="main">
        ...
        @if ($sliders->count())
            <section class="group relative mx-auto mb-10 max-w-6xl overflow-hidden rounded-[2.5rem] shadow-lg ring-1 ring-slate-900/5">
                ...
                <div id="slide-container" class="relative h-[380px] md:h-[320px]">
                    @foreach ($sliders as $index => $slider)
                        <div class="{{ $loop->first ? 'opacity-100 z-10' : 'opacity-0 z-0' }} absolute inset-0 h-full w-full transition-all duration-700 ease-in-out" id="slide-{{ $index }}">
                            ...
                            {{-- Gambar --}}
                            <img src="{{ asset('storage/' . $slider->image) }}" alt="{{ $slider->title }}" class="h-full w-full object-cover object-center transition-transform duration-[3000ms] ease-out group-hover:scale-105">
